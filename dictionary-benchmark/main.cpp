...
void benchmark(Dictionary* dict, const std::string& label, int N, std::ofstream& csv) {
    ...
    for (int i = 0; i < M && i < static_cast<int>(keys.size()); ++i) {
        dict->remove(keys[i]);
    }
    ...
    long long avg_remove_ns = total_remove_time_ns / std::min(M, N);


    std::cout << label << " [" << "N=" << N << "] - Insert: " << insert_time << "us, Avg Lookup: " << avg_lookup_ns << "ns, Avg Remove: " << avg_remove_ns << "ns\n";
